...
import { createWork } from '@/app/lib/works/actions';
import { useFormState } from 'react-dom';

export default function Form({ projects, employees }: { projects: ProjectField[], employees: EmployeeField[] }) {
  const initialState = { message: null, errors: {} };
  const [state, dispatch ] = useFormState(createWork, initialState);

  return (
    <form id="create-work-form" action={dispatch}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        {/* Hours per day */}
        <div className="mb-4">
          <label htmlFor="hours-per-day" className="mb-2 block text-sm font-medium">
            Choose a hours per day
          </label>
          <div className="relative mt-2 rounded-md">
            <div className="relative">
              <input
                ...
                className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                aria-describedby="hours-per-day-error"
                step="1"
              />
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <Button type="submit">Create Wost segment</Button>
      </div>
    </form>
  );
}
